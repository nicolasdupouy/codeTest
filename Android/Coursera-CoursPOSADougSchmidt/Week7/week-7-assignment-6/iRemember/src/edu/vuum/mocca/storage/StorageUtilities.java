...
public class StorageUtilities {
	...
	// Constant that denote whether a file should be stored publicly or
	// privately
	// CHANGE HERE !!
	public static final int SECURITY_PUBLIC = 0; // Line 24
	public static final int SECURITY_PRIVATE = 1;

	/*public static enum SecurityEnum {
		PRIVATE, PUBLIC;
	}*/
	 ...
	 * the security level.
	 * If the security is private, we store it in app-specific private memory.
	 * If it is public, we
	 * store the file on external storage. Android has different directories in
	 * external storage for
	 * each media type, so we choose the directory depending on the media type
	 * parameter.
	 * 
	 ...
	 * @param type
	 *            The media type that's being stored (determines file location
	 *            and name
	 *            if not specified)
	 * @param security
	 *            How securely we should store the temporary files.
	 *            We can store it on the SD card or in private app memory.
	 * @param name
	 *            The name of the file to be created. If null, we generate a
	 *            name based on
	 *            the current time and media type.
	 * @return A File reference to a newly created temporary file
	 */
	public static File getOutputMediaFile(Context context, int type, /*SecurityEnum*/int security, String name) {
		Log.d(LOG_TAG, "getOutputMediaFile() type:" + type);

		// Get the current time stamp
		String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US) // Line
																				// 56
				.format(new Date());
		...
		// CHANGE HERE !! (the default behaviour is private)
		// If security is public, store the file in a public directory depending
		// on its media type.
		if (security == SECURITY_PUBLIC /*SecurityEnum.PUBLIC*/) { // Line 76
			switch (type) {
		...
		// Otherwise, store it in the app's private directory.
		else {
		...
		if (name != null && storageDir != null) {
			outputFile = new File(storageDir.getPath() + File.separator + name); // Line
																					// 92
		}
	 ...
	 *            The context of the calling component
	 * @param type
	 *            The media type that's being stored (determines file name and
	 *            location)
	 * @param security
	 *            How securely we should store the temporary files.
	 *            We can store it on the SD card or in private app memory.
	 ...
	 */
	public static Uri getOutputMediaFileUri(Context context, int type, /*SecurityEnum*/int security, String name) {
